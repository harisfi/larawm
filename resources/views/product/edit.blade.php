        ...
        <div class="col-md-8">
            <div class="card mb-4">
                ...
                <div class="card-body px-0 pt-0 pb-2">
                    <form action="{{ route('product.update', $product) }}" method="POST" enctype="multipart/form-data" class="row m-3">
                        @method('PUT')
                        @csrf
                        <div class="col-12 form-group required">
                            <label for="code" class="form-control-label">Code</label>
                            <input name="code" id="code" class="form-control" type="text" placeholder="123456" value="{{ old('code', $product->code) }}" required>
                            @error('code')
                        ...
                        <div class="col-12 form-group required">
                            ...
                            <input name="name" id="name" class="form-control" type="text" placeholder="Product 1" value="{{ old('name', $product->name) }}" required>
                            @error('name')
                                <p class="text-sm text-danger ms-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="col-md-6 form-group required">
                            ...
                            <input name="stock" id="stock" type="number" class="form-control" placeholder="5" value="{{ old('stock', $product->stock) }}" required>
                            @error('stock')
                                <p class="text-sm text-danger ms-1">{{ $message }}</p>
                            @enderror
                        </div>
